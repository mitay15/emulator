...
Запуск (из корня проекта, где лежит папка aaps_emulator):
  python -m aaps_emulator.tools.generate_reference_csv --out aaps_emulator/tests/reference_generated.csv --logs logs
...
def generate(out_path, logs_dir):
    ...
    with open(out_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["idx","ts_s","aaps_eventual","aaps_rate","aaps_duration","aaps_insreq"])
        for r in rows:
            writer.writerow([
                r.get("idx", ""),
                r.get("ts_s", ""),
...
def main():
    ...
    p.add_argument("--out", default="aaps_emulator/tests/reference_generated.csv", help="Output CSV path")
