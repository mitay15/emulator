...
def plot_predbg_diff_single_block(
    ...
    ax1.plot(x, aaps_pred, label="AAPS (Kotlin)", color="blue", linewidth=1.5)
    ax1.plot(x, py_pred, label="Python emulator", color="orange", linewidth=1.5)
    ax1.set_ylabel("BG")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output PNG path (default: reports/predbg_diff/<name>.png)",
    )
    ...
    diffs = data.get("predBGs_diff") or []
    ...
    if args.out:
        ...
        out_dir = Path("reports") / "predbg_diff"
        out_dir.mkdir(parents=True, exist_ok=True)
        out_path = out_dir / f"{mismatch_path.stem}.png"

    title = f"Predicted BG diff — {mismatch_path.name}"
    plot_predbg_diff_single_block(aaps_pred, py_pred, diffs, out_path, title=title)
